"""LRC 組裝(純函式,不依賴 aeneas,可獨立單元測試)。

aeneas 的對齊結果以「秒(浮點)」表示每行起始時間,本模組把它格式化為
Seek Player 既有 parser 認得的標準 LRC(`[mm:ss.xx]` 每行,百分之一秒)。
與 `lib/features/lyrics/lyrics_parser.dart` 的小數秒慣例一致(2 位 → 百分秒)。
"""
