...
問題:同一次請求會跨多個模組(``main`` / ``aligner``)各自輸出 log;在 GCP Logs
Explorer 裡這些行會與其他請求交錯,難以判斷某行屬於哪一次請求、哪個端點、哪個階段。
...
GCP Logs Explorer 用法:以該請求 ID 字串搜尋,即可撈出單次請求從「開始 → 下載 →
對齊 → 完成」的完整生命週期。
"""
...
def configure_logging(level: int = logging.INFO) -> None:
    ...
    取代 ``logging.basicConfig``;對 root 設定,故 ``main`` / ``aligner`` 等所有
    logger 都會套用同一格式與 tag。
    """
